...
* The test score percentile calculations depend on the precise calculation method used. If the package output does not match your local version (but is very close), you can adjust the method used by R to try to get a match. See the help page for more details `produce_adm_report()`.  
